...
vec4 getFragColor() {
	...
	// 高光部分的分界值.
	float highlightThreshold = 0.6;
  
	// 调整的强度, 0~1.
	float intensity = iPercent / 100.0;
  
	// 归属于高光部分的比例, 0~1, 全白为 1, 分界值为 0.
	float highlightRate = linearstep(hsl.z, highlightThreshold, 1.0);

	// 按照归属于高光的比例调整亮度.
	hsl.z = adjustLightness(hsl.z, intensity * highlightRate);
