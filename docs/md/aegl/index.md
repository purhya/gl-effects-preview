...
- **Plan to do but may never**
	- 裁剪与遮盖预测 (Cropping and Masking Prediction) - 对于会被裁剪以及遮挡的元素, 将最终的绘制区域 (包括因为时间轴移动而产生的区域移动求并), 然后反向投射到中间绘制步骤来引导缓存分配. 例如一张 1080p 的图片, 经过调色, 取其 100\*100 来绘制. 那么分配用于处理调色的缓存只需要 100\*100 即可, 极大提升复杂的拼贴块的绘制效率, 例如 (#aegl/1107).
	- 静态内容预测 (Static Content Prediction) - 对于不随时间变化的内容, 应当将其缓存; 对于随时间变化, 但是仅是位置变化的内容, 可以预测其在整个场景的可能变化范围, 提前将完整内容绘制, 然后跟随时间变化来设置采样位置即可.
...
此项目的首要目标是实时预览, 其次是直接编码视频并且输出. 只可惜 wasm 版本的 ffmpeg 只有普通版本的 5% 左右的编码速度. 这导致 (在我的台式机上) 1080P 30帧的编码时长达到视频时长的 3 倍.

此外, 大部分模版都嵌有视频素材, 这些视频素材需要经由 ffmpeg 导出为帧图像并编码, 再由浏览器解码. 如果视频素材是透明的, 还需要额外编解码 Alpha 通道. 经此 (在我的台式机上) 1080P 30帧的带视频素材的模版的编码时长轻松达到视频时长的 15 倍.
