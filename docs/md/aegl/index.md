...
- **Plan to do but may never**
	- 裁剪与遮盖预测 (Cropping and Masking Prediction) - 对于会被裁剪以及遮挡的元素, 将最终的绘制区域 (包括因为时间轴移动而产生的区域移动求并), 然后反向投射到中间绘制步骤来引导缓存分配. 例如一张 1080p 的图片, 经过调色, 取其 100*100 来绘制. 那么分配用于处理调色的缓存只需要 100*100 即可, 极大提升复杂的拼贴块的绘制效率.
