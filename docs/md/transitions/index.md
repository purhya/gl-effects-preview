...
在之前, 我认为涉及到几何变换的转场改为通过在顶点着色器乘以变换矩阵来实现, 这样插值可以使用 OpenGL 内建的 [MSAA](https://zhuanlan.zhihu.com/p/32823370) 自动完成.

但是后来我了解了 [滤波](#math/filtering) 的原理, 明白了在片元着色器里面处理几何变换的平滑也比较简单.

不过目前已经通过顶点着色器写成的转场在这里还会继续保留作为案例, 需要的时候可以很快地被转为片元着色器实现.
